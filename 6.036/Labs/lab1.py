...
#1) EVALUATING LEARING METHODS


#1.1)Evaluating a classifier

#Testing the classifier with the data points it was initially trained on is
#not a good idea, since the classifier was trained on those data points
#and we won't learn anything new(testing error == training error).

#psedocode
##def eval_classifier(h, D_train, G, num_trials):
##    avg_score = 0
##    for i in num_trials:
##        n = #a number that is significantly larger than d
##        X,y = G(n) #make sure there are no data points that were in D_train
##        avg_score+= score(h, X, y)
##    return avg_score/num_trials

#in the best case, it'd return n which means the classifier has zero testing
#error. in the worst case, it'd return 0. if it had 0 training error then it
#means the classifier overfitted D_train, and doesn't generalize well.

#Percy's approach doesn't test how well the classifier performs on unseen
#inputs. By letting G randomly generate new data points the classifier hasn't
#been trained on multiple times and averaging the scores, we can test the
#classifier better and measure its true performance


#1.2)Evaluating a learning algorithm

#He's only doing a single round of testing. It's better to do it several times
#and see how the classifier scores on average. Multiple rounds of testing
#will decrease the chance of a single horrible or exceptionally good performance
#from the classifier biasing our measurment of the learning algorithm's performance

#psedocode
##def eval_learning alg(L,G):
##    num_trials = n? #what is the optimal n and what does it depend on..possibly variance?
##    scores = 0
##    for i in range(num_trials):
##        h = L(X,y)
##        scores+=eval_classifier(h,G))
##    return scores/num_trials

#in the best case, the learning algorithm returns a classifier that has 0 testing
#error in every trial. So, eval_learning alg returns num_trials. in the worst
#case the learning algorithm is no good for our application and eval returns 0

#1.3)Evaluating a learning algorithm with a finite amount of data

#K-fold cross-validation
#since the data is finite, we can divide our finite data into k subsets then
#train h with k-1 subsets and test it on the held back subset. we can repeat this
#k times, each time holding out a new subset and scoring how h does on it.


##3) IMPLEMENT PERCEPTRON

#wrote the whole pset thinking labels is nX1 so i have needlessly transposed 'labels'
#in my code....

#data: dXn and labels:1xN
def perceptron(data, labels, params={}, hook=None):
    ...
    for i in range(T):
        for i in range(n):
            if (np.dot(theta.T, data[:,[i]]) + theta_o)*labels[i] <= 0:                
                theta += (data[:,[i]]*labels[i])
                theta_o += labels[i][0]
                if hook:
                    hook((theta, theta_o))
    return theta, theta_o

###test cases: PERCEPTRON:
...
def averaged_perceptron(data, labels, params={}, hook =None):
    ...
    for i in range(T):
        for i in range(n):
            ...
            thetas += theta
            theta_os += theta_o  
    return (thetas/(n*T), theta_os/(n*T))


###test cases: AVERAGED_PERCEPTRON:
##for datafn in (super_simple_separable, xor, xor_more, big_higher_dim_separable):
##    data, labels = datafn()
##    print(averaged_perceptron(data, labels, {"T": 100}))
...
def eval_classifier(learner, data_train, labels_train, data_test, labels_test, params = {}):
    theta, theta_o = learner(data_train, labels_train, params = params) 
    return score(data_test, labels_test, theta, theta_o)/labels_test.shape[1]

### test cases: EVAL_CLASSIFIER:
##print(eval_classifier(perceptron, data1, labels1, data2, labels2))
##print(eval_classifier(averaged_perceptron, data2, labels2, data2, labels2))


##2.2. Evaluating a learning algorithm using a data source

#add p as an argument for question 5
def eval_learning_alg(learner, data_gen, n_train, n_test, it, params = {}):
    performance = 0
    for i in range(it):
        data_train, labels_train = data_gen(n_train)
        data_test, labels_test = data_gen(n_test)
        performance += eval_classifier\
                       (learner, data_train, labels_train, data_test,\
                        labels_test, params = params)
    return performance/it

### test cases:EVAL_LEARNING_ALG:
##print(eval_learning_alg(perceptron, gen_big_data(), 10, 10, 5))
##print(eval_learning_alg(averaged_perceptron, gen_big_data(), 10, 10, 5))
...
def xval_learning_alg(learner, data, labels, k, params = {}):
    ...
    r = n%k
    labels = labels.T
    k_data = np.split(data[:,0: (n-r)], k, axis = 1)
    k_data[-1] = np.concatenate((k_data[-1], data[:,n-r:n]), axis = 1)
    k_labels = np.split(labels[0:(n-r),:], k, axis = 0)
    k_labels[-1] = np.concatenate((k_labels[-1], labels[n-r:n,:]), axis = 0)

    performance = 0
    for i in range(k):
        data_train = np.concatenate(tuple(k_data[:i]+k_data[i+1:]), axis = 1)
        labels_train = np.concatenate(tuple(k_labels[:i]+k_labels[i+1:]), axis = 0)
        performance += eval_classifier\
                       (learner, data_train, labels_train.T, k_data[i],\
                        k_labels[i].T, params = params)
    return performance/k


### test cases:XVAL_LEARNING_ALG:
##print(xval_learning_alg(perceptron, big_data, big_data_labels, 5, {'T':100}))
##print(xval_learning_alg(averaged_perceptron, big_data, big_data_labels, 5))
...
###plotting
##datafn = (super_simple_separable, xor, xor_more, big_higher_dim_separable)
##learner = (perceptron, averaged_perceptron)
##test_linear_classifier(datafn[3], learner[0], draw = True, refresh = True)
##data, labels = big_higher_dim_separable()
##print (data.shape, labels.shape, perceptron(data, labels))
##def gen_flipped_lin_separable(num_points=20, pflip=0.25,\
...
def test_accuracy(learners, pflips, num_trials):
    accuracy = []
    for alg in learners:
        for p in pflips :
            out = eval_learning_alg\
                    (p, alg, gen_flipped_lin_separable,20, 20, num_trials)
            accuracy.append((alg, p, out))
    return accuracy


pflips = [0.1, 0.25]
learners = [perceptron, averaged_perceptron]
num_trials = 200 #whats a reasonable choice for n

#print (test_accuracy(learners, pflips, num_trials))

#Testing on unseen data

##(perceptron, 0.1, 0.75125)
##(averaged_perceptron, 0.1, 0.80275)
##(perceptron, 0.25, 0.5885) 
##(averaged_perceptron, 0.25, 0.6525)

#Testing on the training data

##(perceptron, 0.1, 0.82025)
##(averaged_perceptron, 0.1, 0.861)
##(perceptron, 0.25, 0.67)
##(averaged_perceptron, 0.25, 0.72075)
